...
const BlogCard = ({
  ...
  const { loading, data, error } = useQuery(GET_USER_BY_ID_QUERY, {
    variables: { id: userId }, // ✅ Corrected variable name
  });
    ...
    <div className="blogCardContainer">
      ...
      >
        ...
        <Stack direction="row" spacing={3} justifyContent="left" mx={3} my={2}>
          {loading ? (
            <div>Loading...</div>
          ) : error || !data?.getUserById ? ( // ✅ Handle missing data
            <div>User Not Found</div>
          ) : (
            <>
              <Avatar
                alt={data.getUserById.name || "Unknown User"} // ✅ Fallback name
                src={data.getUserById.profile_pic || "/default-avatar.png"} // ✅ Fallback avatar
              />
              <div
                style={{
                  textAlign: "left",
